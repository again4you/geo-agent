...
function createTestReport(reportId = "rpt-001", targetId = "target-001") {
...
describe("ArchiveBuilder", () => {
    ...
    describe("build()", () => {
        ...
        it("should write report.json with valid content", () => {
            ...
            const written = JSON.parse(fs.readFileSync(path.join(result.archive_path, "report.json"), "utf-8"));
        ...
        it("should write original files to original/ directory", () => {
            ...
            const content = fs.readFileSync(path.join(result.archive_path, "original", "index.html"), "utf-8");
        ...
        it("should write optimized files to optimized/ directory", () => {
            ...
            const content = fs.readFileSync(path.join(result.archive_path, "optimized", "index.html"), "utf-8");
        ...
        it("should write diff files with .diff extension", () => {
            ...
            const content = fs.readFileSync(path.join(result.archive_path, "diff", "index.html.diff"), "utf-8");
        ...
        it("should return correct total_size_bytes", () => {
            ...
            const result = builder.build(report, new Map([["f.html", origContent]]), new Map([["f.html", optContent]]), new Map([["f.html", diffContent]]));
            const reportJson = JSON.stringify(report, null, 2);
            const expectedSize = Buffer.byteLength(reportJson, "utf-8") +
        ...
        it("should handle multibyte characters in size calculation", () => {
            ...
            const result = builder.build(report, new Map([["index.html", koreanContent]]), new Map(), new Map());
            const reportJson = JSON.stringify(report, null, 2);
            const expectedSize = Buffer.byteLength(reportJson, "utf-8") + Buffer.byteLength(koreanContent, "utf-8");
            expect(result.total_size_bytes).toBe(expectedSize);
        });
        ...
        it("should handle nested file paths in original/", () => {
            ...
            const result = builder.build(report, origFiles, new Map(), new Map());
            const content = fs.readFileSync(path.join(result.archive_path, "original", "subdir", "page.html"), "utf-8");
        ...
        it("should handle nested file paths in optimized/", () => {
            ...
            const result = builder.build(report, new Map(), optFiles, new Map());
            const content = fs.readFileSync(path.join(result.archive_path, "optimized", "deep", "nested", "dir", "file.html"), "utf-8");
        ...
        it("should handle nested file paths in diff/", () => {
            ...
            const result = builder.build(report, new Map(), new Map(), diffs);
            const content = fs.readFileSync(path.join(result.archive_path, "diff", "subdir", "page.html.diff"), "utf-8");
        ...
        it("should handle multiple files across all directories", () => {
            ...
            expect(fs.existsSync(path.join(result.archive_path, "original", "css", "style.css"))).toBe(true);
            expect(fs.existsSync(path.join(result.archive_path, "optimized", "css", "style.css"))).toBe(true);
        });
...
//# sourceMappingURL=archive-builder.test.js.map