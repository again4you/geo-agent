...
import { AppSettingsSchema } from "./config/settings.js";
...
let tmpDirs = [];
function makeTmpDir() {
    const dir = path.join(os.tmpdir(), `geo-bugs-test-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`);
...
function makeCreateTarget(overrides = {}) {
    return {
...
describe("BUG #1: JSON fields returned as strings instead of parsed arrays/objects", () => {
    ...
    it("BUG #1f [FIXED]: create() with llm_priorities array should return array, not string", async () => {
        const db = await createTestDb();
        const repo = new TargetRepository(db);
        const llm_priorities = [{ llm_service: "openai", priority: "critical" }];
...
describe("BUG #5 [FIXED]: DB tables auto-created on startup", () => {
    it("BUG #5 [FIXED]: createDatabase() on fresh DB file should create targets table", async () => {
        ...
        const result = await verifyClient.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name");
