...
import { CostTracker, GeoLLMClient, LLMRequestSchema, LLMResponseSchema, } from "./geo-llm-client.js";
...
describe("GeoLLMClient", () => {
    ...
    describe("chat — error handling", () => {
        it("throws when API key is not set", async () => {
            ...
            await expect(client.chat({ prompt: "Hello", json_mode: false })).rejects.toThrow("No API key configured");
        });
        it("throws for unsupported provider", async () => {
            ...
            const openai = manager.load("openai");
            manager.save({ ...openai, enabled: false });
            await expect(client.chat({ prompt: "Hello", json_mode: false })).rejects.toThrow("not yet supported");
        });
    });
