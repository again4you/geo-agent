...
describe("classifySite", () => {
    ...
    it("identifies manufacturer by Product JSON-LD", () => {
        const html = '<script type="application/ld+json">{"@type":"Product","name":"Galaxy S25"}</script>';
    ...
    it("identifies manufacturer by price information", () => {
        const html = '<html><body><span class="price">$1,299.00</span><meta property="og:product:price" content="1299"></body></html>';
        const result = classifySite(html, "https://shop.example.com/item");
        expect(result.site_type).toBe("manufacturer");
    });
    it("identifies manufacturer by Offer schema and price", () => {
        const html = '<script type="application/ld+json">{"@type":"Offer","price":"499"}</script><p>Price: $499</p>';
        const result = classifySite(html, "https://example.com/buy/laptop");
        expect(result.site_type).toBe("manufacturer");
    });
    // ── Research detection ───────────────────────────────────────
    it("identifies research by ScholarlyArticle", () => {
        const html = '<script type="application/ld+json">{"@type":"ScholarlyArticle","name":"AI Paper"}</script><a href="https://doi.org/10.1234/test">DOI</a>';
    ...
    it("identifies research by DOI and citation tags", () => {
        const html = '<html><head><meta name="citation_title" content="A Study"><meta name="citation_doi" content="10.1234/abc"></head><body><a href="https://doi.org/10.1234/abc">Link</a></body></html>';
        const result = classifySite(html, "https://journal.example.com/paper/123");
        expect(result.site_type).toBe("research");
    });
    it("identifies research by /publications/ URL and ORCID", () => {
        const html = '<html><body><p>Author ORCID: 0000-0001-2345-6789</p><a href="paper.pdf">Download Paper</a></body></html>';
    ...
    it("identifies research by PDF download and paper references", () => {
        const html = '<html><body><a href="/papers/study.pdf">Download paper</a><p>affiliation: MIT</p><meta name="dc.title" content="Study"></body></html>';
    ...
    it("identifies generic when no manufacturer or research signals", () => {
        const html = "<html><body><h1>Welcome to our blog</h1><p>This is a general website.</p></body></html>";
    ...
    it("identifies generic for news article sites", () => {
        const html = '<script type="application/ld+json">{"@type":"Article","headline":"Breaking News"}</script>';
        const result = classifySite(html, "https://news.example.com/article/123");
        expect(result.site_type).toBe("generic");
    });
    it("identifies generic for service/local business sites", () => {
        const html = '<script type="application/ld+json">{"@type":"Service","name":"Consulting"}</script>';
    ...
    it("returns higher confidence with more matching signals", () => {
        ...
        const strongHtml = '<html><body><script type="application/ld+json">{"@type":"Product"}</script><meta property="og:product:price" content="99"><span>$99.00</span></body></html>';
    ...
    it("populates matched_signals for research classification", () => {
        const html = '<meta name="citation_title" content="Study"><a href="https://doi.org/10.1234/abc">DOI</a><p>ScholarlyArticle</p>';
