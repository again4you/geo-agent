...
import { CLASSIFICATION_SIGNALS, 
// Classification
ClassificationSignalSchema, CycleControlSchema, 
// Cycle Control
CycleStopReasonSchema, DEFAULT_SCORING_DIMENSIONS, 
// Evaluation Result
EvaluationResultSchema, 
// Grade & Score
GradeSchema, ProbeResultSchema, 
// Probe
ProbeVerdictSchema, SITE_TYPE_LABELS, 
// Scoring
ScoringDimensionSchema, 
// Site Type
SiteTypeSchema, 
// Template Registry
TEMPLATE_REGISTRY, calculateGrade, calculateOverallScore, getTemplate, shouldStopCycle, } from "./index.js";
// ── Helpers ──────────────────────────────────────────────────────────
...
function makeCycleControl(overrides = {}) {
    return {
        max_cycles: 10,
        target_score: 80,
...
describe("ClassificationSignalSchema", () => {
    ...
    it("accepts confidence boundary values", () => {
        expect(ClassificationSignalSchema.parse({
            site_type: "generic",
            confidence: 0,
            signals: [],
        }).confidence).toBe(0);
...
describe("ScoringDimensionSchema", () => {
    ...
    it("rejects score out of range", () => {
        expect(() => ScoringDimensionSchema.parse({
            id: "S1",
            name: "t",
            weight: 0.1,
            score: -1,
...
describe("DEFAULT_SCORING_DIMENSIONS", () => {
    ...
    it("S1 and S6 names are identical across types", () => {
        const types2 = ["manufacturer", "research", "generic"];
...
describe("EvaluationResultSchema", () => {
    ...
    it("accepts both evaluation targets", () => {
        expect(EvaluationResultSchema.parse(makeEvaluationResult({ evaluation_target: "original" }))
            .evaluation_target).toBe("original");
        expect(EvaluationResultSchema.parse(makeEvaluationResult({ evaluation_target: "clone" }))
            .evaluation_target).toBe("clone");
    });
    it("rejects invalid evaluation target", () => {
        expect(() => EvaluationResultSchema.parse(makeEvaluationResult({ evaluation_target: "modified" }))).toThrow();
    });
    it("accepts all valid grades", () => {
        const grades = ["Excellent", "Good", "Needs Improvement", "Poor", "Critical"];
        for (const g of grades) {
    ...
    it("rejects invalid grade", () => {
        expect(() => EvaluationResultSchema.parse(makeEvaluationResult({ grade: "Bad" }))).toThrow();
    });
    it("accepts empty arrays for findings and improvements", () => {
        const parsed = EvaluationResultSchema.parse(makeEvaluationResult({ key_findings: [], top_improvements: [] }));
    ...
    it("accepts cycle_number > 0 for intermediate results", () => {
        const parsed = EvaluationResultSchema.parse(makeEvaluationResult({
            cycle_number: 3,
...
describe("TEMPLATE_REGISTRY", () => {
    ...
    it("template paths match expected pattern", () => {
        for (const t of TEMPLATE_REGISTRY) {
            expect(t.template_path).toMatch(/^evaluation-templates\/(manufacturer|research|generic)\.md$/);
        }
    });
...
describe("getTemplate", () => {
    ...
    it("throws for unknown site type", () => {
        expect(() => getTemplate("ecommerce")).toThrow("Unknown site type: ecommerce");
    });
...
//# sourceMappingURL=evaluation-templates.test.js.map