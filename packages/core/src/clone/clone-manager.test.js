...
describe("CloneManager", () => {
    ...
    beforeEach(() => {
        tmpDir = path.join(os.tmpdir(), `clone-manager-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    ...
    it("createClone creates directories and files", () => {
        ...
        expect(fs.readFileSync(path.join(clonePath, "original", "index.html"), "utf-8")).toBe(htmlContent);
        expect(fs.readFileSync(path.join(clonePath, "working", "index.html"), "utf-8")).toBe(htmlContent);
    ...
    it("createClone with additionalFiles stores extra files", () => {
        ...
        const clonePath = manager.getClonePath(targetId);
        expect(fs.readFileSync(path.join(clonePath, "original", "styles", "main.css"), "utf-8")).toBe("body { color: red; }");
        expect(fs.readFileSync(path.join(clonePath, "working", "scripts", "app.js"), "utf-8")).toBe("console.log('hello');");
    ...
    it("archiveClone renames directory and sets status to archived", () => {
        ...
        const archivedMeta = JSON.parse(fs.readFileSync(path.join(baseDir, archivedEntry, "metadata.json"), "utf-8"));
    ...
    describe("CloneMetadataSchema", () => {
        ...
        it("accepts all valid status values", () => {
            const statuses = ["creating", "ready", "modifying", "archived", "failed"];
            const now = new Date().toISOString();
...
//# sourceMappingURL=clone-manager.test.js.map