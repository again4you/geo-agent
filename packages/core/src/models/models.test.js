...
import { AccuracyLevelSchema, 
// Agent Configuration
AgentIdSchema, AgentPromptConfigSchema, AnalysisReportSchema, ChangeImpactSchema, ChangeRecordSchema, 
// Change Tracking
ChangeTypeSchema, 
// Target & Content
CompetitorEntrySchema, CompetitorGapSchema, ContentAnalysisSchema, ContentSnapshotSchema, ContextSlotSchema, CrawlerAccessResultSchema, CreateTargetSchema, 
// Agent Memory
EffectivenessIndexSchema, ErrorEventSchema, ErrorTypeSchema, GEO_SCORE_WEIGHTS, 
// GEO Scoring
GeoScorePerLLMSchema, GeoScoreSchema, GeoTimeSeriesSchema, InfoCategorySchema, InfoRecognitionItemSchema, InfoRecognitionPerLLMSchema, InfoRecognitionScoreSchema, LLMAuthConfigSchema, LLMModelConfigSchema, LLMPrioritySchema, LLMProbeSchema, LLMProviderConfigSchema, MachineReadabilitySchema, ModelRoleSchema, NotificationConfigSchema, OAuthConfigSchema, OptimizationPlanSchema, OptimizationTaskSchema, 
// Pipeline & Error Handling
PipelineStageSchema, PipelineStateSchema, 
// LLM
QueryTypeSchema, RetryPolicySchema, SemanticChangeRecordSchema, SeveritySchema, 
// Analysis & Planning
StructureQualitySchema, StructuredDataAuditSchema, TargetProfileSchema, UpdateTargetSchema, ValidationLLMResultSchema, ValidationReportSchema, VerdictSchema, } from "./index.js";
// ── Helpers ──────────────────────────────────────────────────────────
...
describe("InfoRecognition", () => {
    ...
    describe("InfoRecognitionPerLLMSchema", () => {
        ...
        it("accepts null llm_answer", () => {
            expect(InfoRecognitionPerLLMSchema.safeParse(makeInfoRecognitionPerLLM({ llm_answer: null }))
                .success).toBe(true);
        });
        it("accepts null detail", () => {
            expect(InfoRecognitionPerLLMSchema.safeParse(makeInfoRecognitionPerLLM({ detail: null })).success).toBe(true);
        });
        ...
        it("rejects invalid accuracy", () => {
            expect(InfoRecognitionPerLLMSchema.safeParse(makeInfoRecognitionPerLLM({ accuracy: "wrong" }))
                .success).toBe(false);
        });
    });
    describe("InfoRecognitionItemSchema", () => {
        ...
        it("rejects invalid uuid for info_id", () => {
            expect(InfoRecognitionItemSchema.safeParse(makeInfoRecognitionItem({ info_id: "not-a-uuid" }))
                .success).toBe(false);
        });
        it("rejects invalid category", () => {
            expect(InfoRecognitionItemSchema.safeParse(makeInfoRecognitionItem({ category: "INVALID" }))
                .success).toBe(false);
        });
        it("accepts empty llm_results array", () => {
            expect(InfoRecognitionItemSchema.safeParse(makeInfoRecognitionItem({ llm_results: [] })).success).toBe(true);
        });
    ...
    describe("InfoRecognitionScoreSchema", () => {
        ...
        it("accepts overall at boundary 0", () => {
            expect(InfoRecognitionScoreSchema.safeParse(makeInfoRecognitionScore({ overall: 0 })).success).toBe(true);
        });
        it("accepts overall at boundary 100", () => {
            expect(InfoRecognitionScoreSchema.safeParse(makeInfoRecognitionScore({ overall: 100 })).success).toBe(true);
        });
        it("rejects overall > 100", () => {
            expect(InfoRecognitionScoreSchema.safeParse(makeInfoRecognitionScore({ overall: 101 })).success).toBe(false);
        });
        it("rejects negative overall", () => {
            expect(InfoRecognitionScoreSchema.safeParse(makeInfoRecognitionScore({ overall: -1 })).success).toBe(false);
        });
        it("rejects coverage_rate > 1", () => {
            expect(InfoRecognitionScoreSchema.safeParse(makeInfoRecognitionScore({ coverage_rate: 1.1 }))
                .success).toBe(false);
        });
        it("rejects negative accuracy_rate", () => {
            expect(InfoRecognitionScoreSchema.safeParse(makeInfoRecognitionScore({ accuracy_rate: -0.1 }))
                .success).toBe(false);
        });
    });
});
...
describe("LLMProbe", () => {
    ...
    describe("LLMProbeSchema", () => {
        ...
        it("accepts null citation_position", () => {
            expect(LLMProbeSchema.safeParse(makeLLMProbe({ citation_position: null })).success).toBe(true);
        });
        ...
        it("rejects accuracy_vs_source > 1", () => {
            expect(LLMProbeSchema.safeParse(makeLLMProbe({ accuracy_vs_source: 1.5 })).success).toBe(false);
        });
        it("rejects negative accuracy_vs_source", () => {
            expect(LLMProbeSchema.safeParse(makeLLMProbe({ accuracy_vs_source: -0.1 })).success).toBe(false);
        });
        it("rejects invalid datetime for response_at", () => {
            expect(LLMProbeSchema.safeParse(makeLLMProbe({ response_at: "not-a-date" })).success).toBe(false);
        });
...
describe("GeoScore", () => {
    ...
    describe("GeoScoreSchema", () => {
        ...
        it("accepts boundary values at 0", () => {
            expect(GeoScoreSchema.safeParse(makeGeoScore({
                total: 0,
                citation_rate: 0,
                citation_accuracy: 0,
        ...
        it("accepts optional info_recognition", () => {
            expect(GeoScoreSchema.safeParse(makeGeoScore({ info_recognition: makeInfoRecognitionScore() }))
                .success).toBe(true);
        });
...
describe("TargetProfile", () => {
    describe("CompetitorEntrySchema", () => {
        ...
        it("accepts all relationship values", () => {
            for (const rel of ["direct", "indirect", "reference"]) {
                expect(CompetitorEntrySchema.safeParse({ url: URL_VALID, name: "X", relationship: rel }).success).toBe(true);
            }
        });
        it("rejects invalid url", () => {
            expect(CompetitorEntrySchema.safeParse({ url: "not-a-url", name: "X", relationship: "direct" })
                .success).toBe(false);
        });
        it("rejects invalid relationship", () => {
            expect(CompetitorEntrySchema.safeParse({ url: URL_VALID, name: "X", relationship: "enemy" })
                .success).toBe(false);
        });
    });
    describe("LLMPrioritySchema", () => {
        it("accepts valid object", () => {
            expect(LLMPrioritySchema.safeParse({ llm_service: "chatgpt", priority: "critical" }).success).toBe(true);
        });
        it.each(["critical", "important", "nice_to_have", "monitor_only"])("accepts priority: %s", (p) => {
            expect(LLMPrioritySchema.safeParse({ llm_service: "x", priority: p }).success).toBe(true);
        });
        it("rejects invalid priority", () => {
            expect(LLMPrioritySchema.safeParse({ llm_service: "x", priority: "unknown" }).success).toBe(false);
        });
    });
    ...
    describe("TargetProfileSchema", () => {
        ...
        it("rejects invalid url", () => {
            expect(TargetProfileSchema.safeParse({ ...validProfile, url: "not-a-url" }).success).toBe(false);
        });
        ...
        it("rejects invalid status", () => {
            expect(TargetProfileSchema.safeParse({ ...validProfile, status: "deleted" }).success).toBe(false);
        });
...
describe("ContentSnapshotSchema", () => {
    ...
    it("accepts optional geo_score", () => {
        expect(ContentSnapshotSchema.safeParse({ ...validSnapshot, geo_score: makeGeoScore() }).success).toBe(true);
    });
    ...
    it("accepts llm_responses with probes", () => {
        expect(ContentSnapshotSchema.safeParse({
            ...validSnapshot,
            llm_responses: [makeLLMProbe()],
        }).success).toBe(true);
    });
    it("rejects invalid url", () => {
        expect(ContentSnapshotSchema.safeParse({ ...validSnapshot, url: "not-url" }).success).toBe(false);
    });
    it("rejects invalid uuid for snapshot_id", () => {
        expect(ContentSnapshotSchema.safeParse({ ...validSnapshot, snapshot_id: "bad" }).success).toBe(false);
    });
});
// ── ChangeRecord ─────────────────────────────────────────────────────
describe("ChangeRecordSchema", () => {
    ...
    it("accepts null snapshot_after", () => {
        expect(ChangeRecordSchema.safeParse({ ...validRecord, snapshot_after: null }).success).toBe(true);
    });
    ...
    it("rejects invalid change_type", () => {
        expect(ChangeRecordSchema.safeParse({ ...validRecord, change_type: "UNKNOWN" }).success).toBe(false);
    });
    it("rejects invalid triggered_by", () => {
        expect(ChangeRecordSchema.safeParse({ ...validRecord, triggered_by: "cron" }).success).toBe(false);
    });
    it("rejects missing required fields", () => {
...
describe("ChangeImpact", () => {
    ...
    describe("ChangeImpactSchema", () => {
        ...
        it("accepts empty per_llm_impact", () => {
            expect(ChangeImpactSchema.safeParse({ ...validImpact, per_llm_impact: {} }).success).toBe(true);
        });
        ...
        it("rejects negative confidence", () => {
            expect(ChangeImpactSchema.safeParse({ ...validImpact, confidence: -0.1 }).success).toBe(false);
        });
        it("rejects score_before > 100", () => {
            expect(ChangeImpactSchema.safeParse({ ...validImpact, score_before: 101 }).success).toBe(false);
        });
        ...
        it("accepts negative delta (score went down)", () => {
            expect(ChangeImpactSchema.safeParse({ ...validImpact, delta: -10, delta_pct: -15 }).success).toBe(true);
        });
    });
});
// ── GeoTimeSeries ────────────────────────────────────────────────────
describe("GeoTimeSeriesSchema", () => {
    ...
    it("accepts null citation_rank", () => {
        expect(GeoTimeSeriesSchema.safeParse({ ...validEntry, citation_rank: null }).success).toBe(true);
    });
    ...
    it("rejects citation_rate > 1", () => {
        expect(GeoTimeSeriesSchema.safeParse({ ...validEntry, citation_rate: 1.1 }).success).toBe(false);
    });
    ...
    it("rejects non-integer citation_rank", () => {
        expect(GeoTimeSeriesSchema.safeParse({ ...validEntry, citation_rank: 1.5 }).success).toBe(false);
    });
    it("rejects invalid url", () => {
...
describe("AnalysisReport", () => {
    describe("StructureQualitySchema", () => {
        ...
        it("rejects semantic_tag_ratio > 1", () => {
            expect(StructureQualitySchema.safeParse({ ...valid, semantic_tag_ratio: 1.1 }).success).toBe(false);
        });
        it("rejects negative div_nesting_depth", () => {
            expect(StructureQualitySchema.safeParse({ ...valid, div_nesting_depth: -1 }).success).toBe(false);
        });
        it("rejects non-integer div_nesting_depth", () => {
            expect(StructureQualitySchema.safeParse({ ...valid, div_nesting_depth: 2.5 }).success).toBe(false);
        });
    });
    describe("CrawlerAccessResultSchema", () => {
        ...
        it("rejects non-integer http_status", () => {
            expect(CrawlerAccessResultSchema.safeParse({ ...valid, http_status: 200.5 }).success).toBe(false);
        });
    });
    describe("MachineReadabilitySchema", () => {
        ...
        it.each(["A", "B", "C", "F"])("accepts grade: %s", (grade) => {
            expect(MachineReadabilitySchema.safeParse({
                grade,
    ...
    describe("ContentAnalysisSchema", () => {
        ...
        it.each(["technical", "general", "simplified"])("accepts readability_level: %s", (level) => {
            expect(ContentAnalysisSchema.safeParse({ ...valid, readability_level: level }).success).toBe(true);
        });
        ...
        it("rejects content_density > 100", () => {
            expect(ContentAnalysisSchema.safeParse({ ...valid, content_density: 101 }).success).toBe(false);
        });
        it("rejects topic_alignment > 1", () => {
            expect(ContentAnalysisSchema.safeParse({ ...valid, topic_alignment: 1.1 }).success).toBe(false);
        });
    });
    describe("StructuredDataAuditSchema", () => {
        ...
        it("accepts null meta_description", () => {
            expect(StructuredDataAuditSchema.safeParse({ ...valid, meta_description: null }).success).toBe(true);
        });
        it("rejects schema_completeness > 1", () => {
            expect(StructuredDataAuditSchema.safeParse({ ...valid, schema_completeness: 1.1 }).success).toBe(false);
        });
    });
    describe("CompetitorGapSchema", () => {
        ...
        it("accepts null competitor_geo_score", () => {
            expect(CompetitorGapSchema.safeParse({ ...valid, competitor_geo_score: null }).success).toBe(true);
        });
        it("rejects invalid url", () => {
            expect(CompetitorGapSchema.safeParse({ ...valid, competitor_url: "bad" }).success).toBe(false);
        });
    });
    describe("AnalysisReportSchema", () => {
        ...
        it("rejects invalid nested structure", () => {
            expect(AnalysisReportSchema.safeParse({
                ...fullReport,
...
describe("OptimizationPlan", () => {
    describe("OptimizationTaskSchema", () => {
        ...
        it("accepts null target_element", () => {
            expect(OptimizationTaskSchema.safeParse({ ...validTask, target_element: null }).success).toBe(true);
        });
        it.each(["pending", "in_progress", "completed", "skipped", "failed"])("accepts status: %s", (s) => {
...
describe("ValidationReport", () => {
    describe("ValidationLLMResultSchema", () => {
        ...
        it("accepts null rank_position_avg", () => {
            expect(ValidationLLMResultSchema.safeParse({ ...valid, rank_position_avg: null }).success).toBe(true);
        });
        it("rejects citation_rate > 1", () => {
            expect(ValidationLLMResultSchema.safeParse({ ...valid, citation_rate: 1.5 }).success).toBe(false);
        });
        it("rejects negative citation_accuracy", () => {
            expect(ValidationLLMResultSchema.safeParse({ ...valid, citation_accuracy: -0.1 }).success).toBe(false);
        });
    });
...
describe("EffectivenessIndexSchema", () => {
    ...
    it("accepts null llm_service", () => {
        expect(EffectivenessIndexSchema.safeParse(makeEffectivenessIndex({ llm_service: null })).success).toBe(true);
    });
    it("rejects negative sample_count", () => {
        expect(EffectivenessIndexSchema.safeParse(makeEffectivenessIndex({ sample_count: -1 })).success).toBe(false);
    });
    it("rejects non-integer sample_count", () => {
        expect(EffectivenessIndexSchema.safeParse(makeEffectivenessIndex({ sample_count: 1.5 })).success).toBe(false);
    });
    it("rejects success_rate > 1", () => {
        expect(EffectivenessIndexSchema.safeParse(makeEffectivenessIndex({ success_rate: 1.1 })).success).toBe(false);
    });
    it("rejects negative success_rate", () => {
        expect(EffectivenessIndexSchema.safeParse(makeEffectivenessIndex({ success_rate: -0.1 })).success).toBe(false);
    });
    it("accepts negative avg_delta and worst_delta", () => {
        expect(EffectivenessIndexSchema.safeParse(makeEffectivenessIndex({ avg_delta: -3.5, worst_delta: -10 })).success).toBe(true);
    });
    it("rejects invalid url", () => {
        expect(EffectivenessIndexSchema.safeParse(makeEffectivenessIndex({ url: "bad" })).success).toBe(false);
    });
    it("rejects invalid change_type", () => {
        expect(EffectivenessIndexSchema.safeParse(makeEffectivenessIndex({ change_type: "INVALID" }))
            .success).toBe(false);
    });
});
...
describe("SemanticChangeRecordSchema", () => {
    ...
    it("accepts empty embedding array", () => {
        expect(SemanticChangeRecordSchema.safeParse(makeSemanticChangeRecord({ embedding: [] })).success).toBe(true);
    });
    ...
    it("rejects non-number in embedding", () => {
        expect(SemanticChangeRecordSchema.safeParse(makeSemanticChangeRecord({ embedding: ["a", "b"] }))
            .success).toBe(false);
    });
    it("rejects invalid impact_verdict", () => {
        expect(SemanticChangeRecordSchema.safeParse(makeSemanticChangeRecord({ impact_verdict: "unknown" }))
            .success).toBe(false);
    });
    it("rejects missing required fields", () => {
...
describe("AgentPromptConfig", () => {
    ...
    describe("AgentPromptConfigSchema", () => {
        ...
        it("accepts null model_preference", () => {
            expect(AgentPromptConfigSchema.safeParse({ ...valid, model_preference: null }).success).toBe(true);
        });
        ...
        it("rejects negative temperature", () => {
            expect(AgentPromptConfigSchema.safeParse({ ...valid, temperature: -0.1 }).success).toBe(false);
        });
        it("rejects invalid agent_id", () => {
            expect(AgentPromptConfigSchema.safeParse({ ...valid, agent_id: "hacker" }).success).toBe(false);
        });
...
describe("ErrorEvent", () => {
    ...
    describe("ErrorEventSchema", () => {
        ...
        it("accepts context with mixed value types", () => {
            expect(ErrorEventSchema.safeParse({
                ...valid,
...
describe("LLMProviderConfig", () => {
    describe("OAuthConfigSchema", () => {
        ...
        it("accepts explicit tokens", () => {
            expect(OAuthConfigSchema.safeParse({
                ...valid,
    ...
    describe("LLMAuthConfigSchema (discriminated union)", () => {
        ...
        it("rejects unknown method", () => {
            expect(LLMAuthConfigSchema.safeParse({ method: "basic", username: "u", password: "p" }).success).toBe(false);
        });
    ...
    describe("LLMModelConfigSchema", () => {
        ...
        it("rejects negative cost", () => {
            expect(LLMModelConfigSchema.safeParse({
                ...valid,
    ...
    describe("LLMProviderConfigSchema", () => {
        ...
        it("rejects non-positive rate_limit values", () => {
            expect(LLMProviderConfigSchema.safeParse({
                ...valid,
...
describe("PipelineState", () => {
    ...
    describe("PipelineStateSchema", () => {
        ...
        it("accepts fully populated object", () => {
            expect(PipelineStateSchema.safeParse({
                ...valid,
                stage: "FAILED",
                completed_at: NOW,
        ...
        it("rejects invalid datetime for started_at", () => {
            expect(PipelineStateSchema.safeParse({ ...valid, started_at: "not-a-date" }).success).toBe(false);
        });
    });
