...
import { runAnalysis } from "./analysis-agent.js";
// ── Mock CrawlData ──────────────────────────────────────────
...
// ── Mock Dependencies ───────────────────────────────────────
function makeDeps(crawlOverrides) {
...
describe("Analysis Agent", () => {
    ...
    describe("runAnalysis — AnalysisReport structure", () => {
        it("report has valid UUID report_id", async () => {
            const deps = makeDeps();
            const result = await runAnalysis(defaultInput, deps);
            expect(result.report.report_id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
        });
    ...
    describe("runAnalysis — structure quality computation", () => {
        it("detects semantic HTML tags", async () => {
            ...
            expect(result.report.machine_readability.structure_quality.semantic_tag_ratio).toBeGreaterThan(0.5);
        });
        it("detects heading hierarchy", async () => {
            const deps = makeDeps();
            const result = await runAnalysis(defaultInput, deps);
            // Has h1 and h2
            expect(result.report.machine_readability.structure_quality.heading_hierarchy_valid).toBe(true);
        });
        it("computes text-to-markup ratio", async () => {
            const deps = makeDeps();
            const result = await runAnalysis(defaultInput, deps);
            expect(result.report.machine_readability.structure_quality.text_to_markup_ratio).toBeGreaterThan(0);
            expect(result.report.machine_readability.structure_quality.text_to_markup_ratio).toBeLessThan(1);
        });
    });
});
describe("Analysis Agent — with LLM content quality assessment", () => {
    const mockLLMAssessment = {
        ...
        information_gaps: [
            { category: "pricing", description: "No price info", importance: "high" },
        ],
...
//# sourceMappingURL=analysis-agent.test.js.map