...
import { PROBE_DEFINITIONS, runProbes, } from "./synthetic-probes.js";
...
describe("Synthetic Probes", () => {
    ...
    describe("runProbes — basic execution", () => {
        it("runs all 8 probes", async () => {
            const chat = mockChatLLM("Samsung Galaxy S25 Ultra는 삼성의 최신 스마트폰입니다. samsung.com에서 확인하세요.");
        ...
        it("runs selected probes only", async () => {
            ...
            const result = await runProbes(defaultContext, { chatLLM: chat }, { probeIds: ["P-01", "P-04"], delayMs: 0 });
    ...
    describe("runProbes — citation detection", () => {
        it("detects citation by domain name", async () => {
            ...
            const result = await runProbes(defaultContext, { chatLLM: chat }, { probeIds: ["P-01"], delayMs: 0 });
    ...
    describe("runProbes — verdict determination", () => {
        it("PASS when cited and high accuracy", async () => {
            const chat = mockChatLLM("Samsung Galaxy S25 Ultra는 삼성의 플래그십 스마트폰으로, 가전 분야에서도 유명합니다. samsung.com에서 Galaxy S25 Ultra의 상세 스펙을 확인할 수 있습니다. 스마트폰 시장에서 Galaxy 브랜드는 높은 인지도를 자랑합니다.");
            const result = await runProbes(defaultContext, { chatLLM: chat }, { probeIds: ["P-04"], delayMs: 0 });
    ...
    describe("runProbes — error handling", () => {
        ...
        it("continues after individual probe failure", async () => {
            ...
            const result = await runProbes(defaultContext, { chatLLM: chat }, { probeIds: ["P-01", "P-02"], delayMs: 0 });
            expect(result.probes).toHaveLength(2);
            expect(result.probes[0].verdict).toBe("FAIL");
    ...
    describe("runProbes — accuracy estimation", () => {
        it("higher accuracy with more topic matches", async () => {
            const chatWithTopics = mockChatLLM("Samsung Galaxy S25 Ultra 스마트폰은 가전 분야의 Galaxy 시리즈입니다.");
            const chatNoTopics = mockChatLLM("일반적인 제품 정보입니다.");
            const r1 = await runProbes(defaultContext, { chatLLM: chatWithTopics }, { probeIds: ["P-01"], delayMs: 0 });
            const r2 = await runProbes(defaultContext, { chatLLM: chatNoTopics }, { probeIds: ["P-01"], delayMs: 0 });
        ...
        it("higher accuracy with longer responses", async () => {
            ...
            const r1 = await runProbes(defaultContext, { chatLLM: mockChatLLM(longResponse) }, { probeIds: ["P-01"], delayMs: 0 });
            const r2 = await runProbes(defaultContext, { chatLLM: mockChatLLM(shortResponse) }, { probeIds: ["P-01"], delayMs: 0 });
            expect(r1.probes[0].accuracy).toBeGreaterThanOrEqual(r2.probes[0].accuracy);
        });
    });
});
//# sourceMappingURL=synthetic-probes.test.js.map