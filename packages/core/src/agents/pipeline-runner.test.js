...
let tmpDirs = [];
function makeTmpDir() {
    const dir = path.join(os.tmpdir(), `geo-pipeline-runner-${crypto.randomBytes(8).toString("hex")}`);
...
function mockChatLLM() {
    return vi.fn().mockImplementation(async (req) => ({
...
describe("Pipeline Runner — Synthetic Probes", () => {
    ...
    it("tracks probe results with stageCallbacks", async () => {
        ...
        const full = analyzingResult?.resultFull;
        expect(full).toBeDefined();
        expect(full.synthetic_probes).toBeDefined();
        const probes = full.synthetic_probes;
