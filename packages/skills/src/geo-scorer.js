...
function scoreS1Crawlability(data) {
	let score = 0;
	const details = [];
...
function scoreS2StructuredData(data) {
	let score = 0;
	const details = [];
	// JSON-LD presence
	if (data.json_ld.length > 0) {
		...
		const types = data.json_ld.map((ld) => ld["@type"]).filter(Boolean);
		if (types.length > 0) {
			score += 15;
...
function scoreS3ContentReadability(data) {
	let score = 0;
	const details = [];
...
function scoreS4FactDensity(data) {
	let score = 0;
	const details = [];
	const html = data.html;
...
function scoreS5BrandMessage(data) {
	let score = 0;
	const details = [];
	// Brand in title
...
function scoreS6AIInfrastructure(data) {
	let score = 0;
	const details = [];
	// llms.txt
	if (data.llms_txt) {
...
function scoreS7ContentNavigation(data) {
	let score = 0;
	const details = [];
	// Breadcrumbs
...
export const geoScorerSkill = {
	...
	async execute(context, params) {
		...
		try {
			const crawlData = params.crawl_data;
			if (!crawlData) {
				return {
					success: false,
...
//# sourceMappingURL=geo-scorer.js.map
