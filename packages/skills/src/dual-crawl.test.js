...
import { _parsers } from "./dual-crawl.js";
const { extractTitle, extractMetaTags, extractCanonical, extractJsonLd, extractLinks, getBaseUrl } = _parsers;
describe("Dual Crawl Parsers", () => {
    describe("getBaseUrl", () => {
        it("extracts base URL from full URL", () => {
            expect(getBaseUrl("https://www.samsung.com/smartphones/galaxy-s25/")).toBe("https://www.samsung.com");
        });
...
describe("Dual Crawl Skill Interface", () => {
    ...
    it("skill execute returns error for invalid URL", async () => {
        const { dualCrawlSkill } = await import("./dual-crawl.js");
        const result = await dualCrawlSkill.execute({ target_id: "t1", target_url: "http://localhost:99999/nonexistent", workspace_dir: "/tmp" }, { timeout: 1000 });
        expect(result.success).toBe(false);
        expect(result.error).toBeTruthy();
        expect(result.duration_ms).toBeGreaterThanOrEqual(0);
    });
});
//# sourceMappingURL=dual-crawl.test.js.map