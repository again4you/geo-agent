...
import { geoScorerSkill, scoreTarget } from "./geo-scorer.js";
// Helper: create a minimal CrawlData
function makeCrawlData(overrides = {}) {
    return {
...
<body>
	...
	<footer>Footer</footer>
        ...
        sitemap_xml: '<?xml version="1.0"?><urlset><url><loc>https://www.samsung.com/</loc></url></urlset>',
        json_ld: [
            {
                "@type": "Product",
...
describe("Dimension scoring details", () => {
    ...
    describe("S2: Structured Data", () => {
        it("scores higher with JSON-LD", () => {
            const withLd = scoreTarget(makeCrawlData({
        ...
        it("scores higher with OG tags", () => {
            const withOg = scoreTarget(makeCrawlData({
                meta_tags: { "og:title": "Test", "og:description": "Desc" },
            }));
            const without = scoreTarget(makeCrawlData());
            expect(withOg.dimensions.find((d) => d.id === "S2").score).toBeGreaterThan(without.dimensions.find((d) => d.id === "S2").score);
        });
    });
    describe("S3: Content Readability", () => {
        it("scores higher with semantic HTML", () => {
            const withSemantic = scoreTarget(makeCrawlData({
                html: "<html><body><main><article><section><h1>Title</h1><h2>Sub</h2><nav>Nav</nav></section></article></main></body></html>",
            }));
            const without = scoreTarget(makeCrawlData({
                html: "<html><body><div>Hello</div></body></html>",
            }));
            expect(withSemantic.dimensions.find((d) => d.id === "S3").score).toBeGreaterThan(without.dimensions.find((d) => d.id === "S3").score);
        });
    });
    describe("S4: Fact Density", () => {
        it("scores higher with numbers and units", () => {
            const withFacts = scoreTarget(makeCrawlData({
            ...
            const without = scoreTarget(makeCrawlData());
            expect(withFacts.dimensions.find((d) => d.id === "S4").score).toBeGreaterThan(without.dimensions.find((d) => d.id === "S4").score);
        });
    });
    describe("S5: Brand Message", () => {
        it("scores higher with brand schema", () => {
            const withBrand = scoreTarget(makeCrawlData({
                json_ld: [{ "@type": "Organization", name: "Samsung" }],
                meta_tags: { "og:site_name": "Samsung" },
                title: "Samsung",
            }));
            const without = scoreTarget(makeCrawlData());
            expect(withBrand.dimensions.find((d) => d.id === "S5").score).toBeGreaterThan(without.dimensions.find((d) => d.id === "S5").score);
        });
    });
    describe("S6: AI Infrastructure", () => {
        it("scores higher with llms.txt", () => {
            ...
            const without = scoreTarget(makeCrawlData());
            expect(withAI.dimensions.find((d) => d.id === "S6").score).toBeGreaterThan(without.dimensions.find((d) => d.id === "S6").score);
        });
    });
    describe("S7: Content Navigation", () => {
        it("scores higher with breadcrumbs and nav", () => {
            const withNav = scoreTarget(makeCrawlData({
            ...
            const without = scoreTarget(makeCrawlData());
            expect(withNav.dimensions.find((d) => d.id === "S7").score).toBeGreaterThan(without.dimensions.find((d) => d.id === "S7").score);
        });
    });
});
describe("GEO Scorer Skill Interface", () => {
    ...
    it("returns error when no crawl_data provided", async () => {
        const result = await geoScorerSkill.execute({ target_id: "t1", target_url: "https://example.com", workspace_dir: "/tmp" }, {});
        expect(result.success).toBe(false);
        expect(result.error).toContain("crawl_data");
    });
    it("scores when crawl_data is provided", async () => {
        const result = await geoScorerSkill.execute({ target_id: "t1", target_url: "https://example.com", workspace_dir: "/tmp" }, { crawl_data: makeRichCrawlData() });
...
//# sourceMappingURL=geo-scorer.test.js.map