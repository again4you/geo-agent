...
1. **Dashboard 프론트엔드 고도화** — 평가 결과 실시간 표시, WebSocket 연동, 파이프라인 실행 UI
2. **Monitoring Agent 구현** — 주기적 GEO 추적 + 이상 감지
3. **Agent Memory** — EffectivenessIndex 쿼리, 유사 사례 검색 도구
4. **OAuth 콜백 API** — `/api/auth/callback` 엔드포인트 + Dashboard OAuth UI
5. **schema-builder / content-optimizer 스킬** — LLM 기반 JSON-LD 생성 + 콘텐츠 개선
