...
| 테스트 | vitest | |
| LLM 엔진 | @mariozechner/pi-ai | 멀티 프로바이더 통합 (OpenAI/Anthropic/Google/Perplexity/Azure) |
...
#### Rich 10-Tab Dashboard Output Schema ✅
- packages/core/src/agents/analysis/rich-analysis-schema.ts — 10탭 대시보드 출력 스키마
  - OverviewTab: 4-카드 요약 + 7차원 + LLM 접근성 + 강점/약점/기회
  - CrawlabilityTab: 봇 정책 + 차단/허용 경로 + llms.txt
  - StructuredDataTab: 페이지별 품질 + 스키마 타입 분석 매트릭스
  - ProductsTab: 카테고리별 점수 + 제품 목록 + 스펙 인식도
  - BrandTab: 브랜드 인식 차원 + 마케팅 클레임 검증성
  - PagesTab: 페이지별 점수 + 태그 (good/bad/neutral)
  - RecommendationsTab: 우선순위별 개선안 + 경쟁사 비교
  - EvidenceTab: 실증 데이터 + 스키마 구현 매트릭스 + JS 의존성 상세
  - ProbesTab: P-01~P-08 프로브 결과 + PASS/PARTIAL/FAIL
  - RoadmapTab: 소비자 시나리오 + 취약점 + 개선 기회 매트릭스
- RichAnalysisReport: 전체 10탭 통합 보고서 타입

#### 3개 새 분석 도구 추가 ✅
- analyze_brand_message: 마케팅 클레임 추출 + 감정/검증성 분석 + 브랜드 인식 6차원
- analyze_product_recognition: 카테고리별 제품 데이터 가용성 + 스펙 인식률 (static HTML vs JS)
- collect_evidence: JSON-LD 스니펫 + robots.txt AI 봇 규칙 + 스키마 구현 매트릭스 + JS 의존성 상세

#### SKILL.md v2.0 (samsung_geo_dashboard.html 참조 품질) ✅
- 3-Phase 프로세스: Data Gathering → Analysis & Synthesis → Report Generation
- LLM에게 10탭 JSON 출력 스키마 명시 (각 탭의 정확한 필드 구조)
- 품질 기준: "모든 점수는 도구 결과에 근거", "LLM 크롤러 관점에서 분석"

#### Pipeline 연동 ✅
- ANALYZING 스테이지: LLM provider 설정에서 자동 resolveModel() → runLLMAnalysis()
- richReport (10탭)를 result_full에 포함하여 DB 저장
- RichAnalysisReport mandatory — LLM 실패 시 파이프라인 실패 (fallback 없음)
- LLM agent loop 후 필수 도구 미호출 시 자동 보완 (Score 0 방지)

#### API Key 미설정 시 파이프라인 거부 ✅
- API Key 없으면 즉시 에러 + SSE pipeline:failed + 에러 모달

#### GeoLLMClient → pi-ai 통합 (중복 제거) ✅
- 5개 프로바이더별 SDK 호출 함수 삭제: callOpenAI, callOpenAIResponses, callAnthropic, callGoogle, callAzureOpenAI
- PRICING 테이블 + estimateCost() 삭제 → pi-ai calculateCost() 사용
- isResponsesModel() 삭제 → pi-ai 모델 레지스트리 자동 감지
- 직접 SDK 의존성 제거: openai, @anthropic-ai/sdk, @google/generative-ai
- 새 흐름: GeoLLMClient.chat() → piAiModelFromProvider() → piAiComplete() → pi-ai complete()
- json_mode: pi-ai onPayload 콜백으로 OpenAI/Google response_format 주입
- Perplexity: piAiModelFromProvider()에서 기본 baseUrl 자동 설정
- GPT-5 계열 (gpt-5.x, o3, o4, codex): pi-ai가 v1/responses API 자동 라우팅
- geo-llm-client.ts: 435줄 → 135줄 (순 -300줄)

#### 버그 수정 ✅
- json_mode:true 프롬프트에 'json' 단어 필수 포함 (OpenAI Responses API 요구사항)
- pi-ai Model fallback에서 GPT-5/codex/o3/o4 API 타입 정확 매핑
- LLM agent loop 후 필수 도구(crawl/classify/score/eval) 미호출 시 자동 보완

#### 테스트 ✅
- packages/core/src/skills/skill-loader.test.ts — 8 tests
- packages/core/src/agents/analysis/tools.test.ts — 21 tests
- packages/core/src/llm/pi-ai-bridge.test.ts — 9 tests
- packages/core/src/agents/analysis/llm-analysis-agent.test.ts — 5 tests
...
### Analysis Agent — ARCHITECTURE.md 대비 미구현 항목 (ARCHITECTURE.md 섹션 4.2 참조)

아래 항목들은 ARCHITECTURE.md에 명시된 Analysis Agent 요구사항 중 현재 구현되지 않았거나 차이가 있는 항목입니다.
각 항목은 독립적으로 구현 가능합니다. 관련 코드는 `packages/core/src/agents/analysis/`, `packages/core/src/agents/probes/` 에 있습니다.

| # | 항목 | 심각도 | 현재 상태 | 참조 |
|---|------|--------|-----------|------|
| A-1 | **Explicit 페이지 제공 LLM 추출 테스트** | 높음 | 미구현 | ARCH 4.2 line 295 |
| A-2 | **사이트 종류별 Probe 프롬프트** | 중간 | 미구현 | ARCH 4.2 line 297 |
| A-3 | **Probe 프롬프트 사용자 커스터마이징** | 중간 | 미구현 | ARCH 4.2 line 297 |
| A-4 | **경쟁사 격차 분석 (CompetitorGap)** | 중간 | 빈 배열 | ARCH 4.2 line 293, 4-C.5 |
| A-5 | **InfoRecognitionItem 자동 추출 + 사용자 검토** | 중간 | 빈 배열 | ARCH 4.2 line 301 |
| A-6 | **citation_excerpt, citation_position** | 낮음 | LLMProbe에 미포함 | ARCH 4-C.4 |
| A-7 | **Machine Readability Grade 종합 판정** | 낮음 | overall_score 기반 (JS+시맨틱+크롤러 미종합) | ARCH 4.2-A (4) |
| A-8 | **StructureQuality 필드 보완** | 낮음 | avg_div_depth, has_main_landmark 누락 | ARCH 4.2-A (2) |
| A-9 | **AI 봇 목록 동기화** | 낮음 | Bytespider, cohere-ai 누락 / OAI-SearchBot, Meta-ExternalAgent 추가됨 | ARCH 4.2-A (3) |
| A-10 | **멀티 페이지 크롤링 최대 30페이지** | 낮음 | 현재 20페이지 | ARCH 4.2-A (1) |

#### A-1: Explicit 페이지 제공 LLM 추출 테스트
- **요구사항**: 크롤된 페이지 HTML/콘텐츠를 LLM context로 직접 제공하고, 주요 정보(제품명, 가격, 스펙 등)를 추출할 수 있는지 테스트
- **현재**: Synthetic Probes는 브랜드명 기반 질의만 수행 (LLM의 기존 학습 데이터 의존)
- **구현 방향**: `buildPageContext()`로 페이지 요약 생성 → LLM에 context로 제공 → "이 페이지에서 X를 추출하세요" 형태 프로브
- **관련 코드**: `packages/core/src/agents/probes/synthetic-probes.ts`, `packages/core/src/agents/shared/llm-helpers.ts` (buildPageContext)

#### A-2: 사이트 종류별 Probe 프롬프트
- **요구사항**: manufacturer/research/generic 별로 다른 프로브 질문 세트 사용
- **현재**: `PROBE_DEFINITIONS` 8개가 모든 site_type에 동일
- **구현 방향**: site_type별 `PROBE_DEFINITIONS` 맵 작성 (manufacturer: 제품 스펙/가격 중심, research: 논문/연구 결과 중심, generic: 서비스/정보 중심)
- **관련 코드**: `packages/core/src/agents/probes/synthetic-probes.ts` (PROBE_DEFINITIONS)

#### A-3: Probe 프롬프트 사용자 커스터마이징
- **요구사항**: Dashboard에서 프로브 질문을 사용자가 편집 가능 + Reset to Default 버튼
- **현재**: 하드코딩된 PROBE_DEFINITIONS만 사용
- **구현 방향**: `ProviderConfigManager` 패턴 활용 — 프로브 정의를 JSON 파일로 저장, Dashboard UI에서 편집/리셋 API 제공
- **관련 코드**: `packages/core/src/llm/provider-config.ts` (패턴 참조), `packages/dashboard/src/ui/dashboard.html`

#### A-4: 경쟁사 격차 분석
- **요구사항**: 경쟁 페이지 대비 GEO 격차 분석 → `CompetitorGap[]` 생성
- **현재**: `competitor_gaps: []` (항상 빈 배열)
- **구현 방향**: TargetProfile.competitors URL을 크롤 + 채점 → 차원별 격차 계산
- **관련 코드**: `packages/core/src/agents/analysis/analysis-agent.ts`, `packages/core/src/models/analysis-report.ts`

#### A-5: InfoRecognitionItem 자동 추출
- **요구사항**: JSON-LD + 본문에서 제품/가격/스펙/정책 등 핵심 정보 자동 추출 → 사용자 검토/보완 UI
- **현재**: `extracted_info_items: []` (빈 배열). `geo-eval-extractor.ts`의 `extractProductInfo()`가 유사 기능을 하지만 InfoRecognitionItem 형식이 아님
- **구현 방향**: `extractProductInfo()` 결과를 `InfoRecognitionItem` 형식으로 변환 + Dashboard 검토 UI
- **관련 코드**: `packages/core/src/agents/analysis/geo-eval-extractor.ts` (extractProductInfo)

#### A-6 ~ A-10: 낮은 심각도 항목
- A-6: `SyntheticProbeResult`에 `citation_excerpt`, `citation_position` 필드 추가
- A-7: Grade 산출 시 `js_dependency_ratio` + `semantic_tag_ratio` + 크롤러 차단 상태 종합
- A-8: `computeStructureQuality()`에 `avg_div_depth`, `has_main_landmark` 추가
- A-9: `AI_BOTS` 배열에 Bytespider, cohere-ai 추가
- A-10: `crawlMultiplePages()` maxPages 기본값 20 → 30 변경
...
- **pi-ai 통합**: @mariozechner/pi-ai가 유일한 LLM 호출 경로 — piAiComplete() → complete() (모든 프로바이더)
- **GeoLLMClient**: pi-ai 래퍼 — ProviderConfigManager에서 설정 로드 → piAiModelFromProvider() → piAiComplete()
- **SKILL.md 패턴**: openclaw 호환 스킬 프롬프트 — parseSkillMd(), loadBuiltinSkill()
- **LLM-Driven Analysis Agent**: pi-ai tool-calling agent loop으로 LLM이 분석 도구를 직접 호출 (fallback 없음)
