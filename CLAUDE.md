...
1. **End-to-End 파이프라인 통합** — Orchestrator + Skills + CloneManager + ReportBuilder 연결
2. **Dashboard 프론트엔드 고도화** — 평가 결과 실시간 표시, WebSocket 연동
3. **Analysis Agent 구현** — Dual Crawl + Site Classifier + 초기 평가 로직
4. **Strategy Agent 구현** — GEO Scorer 결과 기반 최적화 전략 생성
5. **Optimization Agent 구현** — 클론 파일 실제 수정 로직
6. **Validation Agent 구현** — 수정 후 재평가 + 이전 점수 비교
7. **OAuth 콜백 API** — `/api/auth/callback` 엔드포인트 + Dashboard OAuth UI
