...
**목적**:
- Target Web Page의 현재 상태를 다각도로 분석 (초기 1회 분석이 대상인 경우)
- Validation Agent에 요청에 의해 Cloned Page에 대한 분석.

**수행 작업**:
- 페이지 크롤링 및 콘텐츠 추출 (HTML, 구조화 데이터, 메타데이터). 단, Validation Agent에 의한 Cloned Page 분석인 경우 주어진 Cloned Page에 대해서만 수행.
- 기존 Schema.org / JSON-LD 마크업 감사
- 콘텐츠 밀도, 명확성, 인용 가능성 점수 산출
- 경쟁 페이지 대비 GEO 격차 분석
- LLM별 인덱싱 현황 파악 (`robots.txt` AI 크롤러 허용 상태, `llms.txt` 존재 여부 확인). 단 Validation Agent에 의한 Cloned Page 분석인 경우 주어진 Cloned Page에 대해 수행. 대상 file이 cloned 혹은 생성되지 않은 경우 web access 로 구함.
- LLM (Claude, GPT, Gemini 중 Available 한 서비스를 이용) 이 선택된 메인 페이지와 크롤된 페이지를 Explicit하게 주어졌을 떄에 주요 정보를 잘 추출하는 지 실제 테스트. 단 Validation Agent에 의한 Cloned Page 분석인 경우 주어진 Cloned Page에 대해 수행.
- LLM (Claude, GPT, Gemini 중 Available 한 서비스를 이용) 이 선택된 메인 페이지가 대표하는 브랜드/회사/Entity를 대상으로 한 Prompt에 대해서 주요 정보를 잘 추출하는 지 실제 테스트. (예: samsung.com 내 특정 페이지나 대표 URL 대신 "삼성전자" 가 주어짐. 단, Validation Agent에 의한 Cloned Page 분석인 경우 주어진 Cloned Page에 대해 수행.
- LLM 을 이용한 실제 테스트를 위한 Prompt 는 실제 사용자가 해당 페이지에 관해 물어볼 만한 주요 항목에 대해 8건 이상 수행. 이 프롬프트는 정의된 사이트 종류별로 다르게 하여 준비하고, 사용자가 바꿀수도 있도록 한다. (Reset하여 predefined prompt로 되돌리기 가능)
- 모든 평가 결과는 저장하여 최종 Evaluation Page에서 상세항목을 모두 조회할 수 있어야 한다. 여러 단계에서 Validation Agent에 의해 수행될 수 있으므로, 각 수행 간 별도로 저장되어, 각 단계별로 어떻게 바뀌는지 조회 되어야 한다.
- 사용된 prompt input/output도 저장하여 차후에 조회할 수 있어야 한다. 단, 길이제한으로 너무 긴 부분을 잘라내는 것은 괜찮다.
...
- 제품 정보를 읽어야 하는 경우 (제조사 페이지) 제품 정보를 구하기 위해 탐색하며, 해당 페이지들이 크롤링 대상으로 포함되어야 한다.

**출력**:
- `AnalysisReport` (JSON 구조화 보고서) — 4-C.5 참조
- 사용된 LLM input/output log
- 평가를 위해 사용된 Page들의 URL (crawling 대상)
