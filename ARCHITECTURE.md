...
- LLM (Claude, GPT, Gemini 중 Available 한 서비스를 이용) 이 선택된 메인 페이지와 크롤된 페이지를 Explicit하게 주어졌을 떄에 주요 정보를 잘 추출하는 지 실제 테스트
- LLM (Claude, GPT, Gemini 중 Available 한 서비스를 이용) 이 선택된 메인 페이지가 대표하는 브랜드/회사/Entity를 대상으로 한 Prompt에 대해서 주요 정보를 잘 추출하는 지 실제 테스트. (예: samsung.com 내 특정 페이지나 대표 URL 대신 "삼성전자" 가 주어짐
...
- 모든 평가 결과는 저장하여 최종 Evaluation Page에서 상세항목을 모두 조회할 수 있어야 한다.
