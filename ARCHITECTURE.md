...
**목적**: 최적화된 **로컬 클론**의 품질을 검증하고 개선 효과를 측정

> **이중 트랙 검증**: 원본 URL은 읽기 전용이므로, 루프 내 검증은 로컬 클론을 대상으로 수행한다.
> LLM이 로컬 클론에 직접 접근할 수 없으므로, **구조적 검증(로컬)** + **LLM 검증(원본 기준선)** 이중 트랙으로 운영한다.

**수행 작업**:

**(1) 구조적 검증 (로컬 클론 대상 — 루프 내 매 반복 수행)**:
- 클론의 DOM 구조 품질 점수 재산출 (시맨틱 태그 비율, heading 계층 등)
- JSON-LD / Schema.org 완성도 검증
- 콘텐츠 밀도, 팩트 밀도 점수 재산출
- 기계 가독성 등급 재산출
- 변경 전/후 구조적 GEO 점수(Structured Score) 비교

**(2) LLM 기준선 검증 (원본 URL 대상 — 초기 1회 + 최종 비교)**:
- 초기 분석 시 원본 URL에 대한 LLM 질의 결과를 기준선(baseline)으로 저장
- 최적화 루프 완료 후, 사용자가 변경사항을 실제 사이트에 반영한 뒤 재검증 트리거 가능
- **정보 인식 검증 (Information Recognition)**: Target Page의 핵심 정보를 LLM이 정확히 인식하는지 검증 — 4-C.3 참조
...
**출력**: `ValidationReport` (구조적 점수 비교, LLM 기준선 데이터, 예측 효과, 권장 후속 조치)
...
## 이중 트랙 검증 원칙
원본 사이트는 읽기 전용이므로 LLM이 클론에 직접 접근할 수 없습니다.
따라서 **구조적 검증(클론 대상)** + **LLM 기준선 비교(원본 대상)** 이중 트랙으로 운영합니다.
...
### 트랙 1: 구조적 검증 (클론 대상 — 매 루프 반복)
1. **DOM 구조 품질**: 시맨틱 태그 비율, heading 계층, div 중첩 깊이 재산출
2. **구조화 데이터 완성도**: JSON-LD, Schema.org 커버리지 재산출
3. **콘텐츠 밀도**: 팩트 밀도, 정보 밀도 점수 재산출
4. **기계 가독성 등급**: js_dependency_ratio, 크롤러 접근성 재산출
5. **구조적 ChangeImpact 산출**: 변경 전/후 구조적 점수 비교, delta 계산

### 트랙 2: LLM 기준선 검증 (원본 URL 대상 — 초기 1회)
1. **인용 테스트**: target_queries + 자동 생성 질의로 각 LLM에 질의
2. **정보 인식 검증**: extracted_info_items 각 항목에 대해 LLM별 정확도 확인
3. **기준선 저장**: 이 결과를 baseline으로 보존하여 최종 비교에 사용

### 트랙 3: 예측 효과 산출
1. 구조적 개선 수치 + Agent Memory 과거 데이터를 기반으로 LLM 인용률 개선 예측
2. 과거 유사 변경의 ChangeImpact 참조하여 예측 신뢰도 산출

## 측정 규칙
1. 구조적 검증은 클론에서 즉시 수행 (대기 불필요)
2. LLM 기준선 검증은 각 질의를 최소 3회 반복하여 평균값 사용
3. llm_priorities에서 critical인 LLM은 질의 수를 2배로 확보
4. confidence가 0.5 미만이면 "측정 불충분" 경고 발행

## 출력 형식
ValidationReport (4-C.7) JSON 형식으로 출력하세요.
구조적 점수 변화와 예측 LLM 효과를 분리하여 보고하세요.
```
...
const analysisAgent = createAgent({
  name: "analysis-agent",
  model: piAi.model("openai", "gpt-4o"),  // 기본값; 대시보드에서 변경 가능
  tools: [
...
| **파일 저장** | 로컬 파일시스템 / S3 호환 | 크롤링 원본, 로컬 클론, HTML diff, 패치, 리포트 Archive |
...
│  │  (에이전트별 시스템 프롬프트 편집, Model/Temp 설정)       │   │
│  │  → 섹션 4-A.5 프롬프트 편집 UI 참조                     │   │
│  │  [💾 저장] [🔄 Reset to Default] [📋 Diff 보기]        │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─ /settings/llm-providers ─────────────────────────────┐   │
│  │  LLM Provider 관리 (API Key / OAuth 설정)              │   │
│  │  기본 오케스트레이션 모델 선택 (GPT-4o 기본)            │   │
│  │  Provider별 연결/테스트/비활성화, 월간 비용 요약         │   │
...
[Orchestrator: TargetProfile 로드 → 파이프라인 초기화]
         ...
         │  │  │    수정된 클론 HTML/JSON-LD                       │
         │  │  │                                                   │
         │  │  ├──▶ [Validation Agent] ★ 클론 구조적 검증          │
         │  │  │         │ 클론의 DOM 구조/JSON-LD/콘텐츠 밀도 재측정│
         │  │  │         │ ContentSnapshot(after) 저장              │
         │  │  │         │ 구조적 ChangeImpact 산출                │
         │  │  │         │ Agent Memory 기반 LLM 효과 예측         │
         │  │  │         ▼                                         │
         │  │  │    ValidationReport + ChangeImpact                │
         │  │  │                                                   │
...
[Analysis Agent 완료 후]
    ...
    │     ├─ index.html           # 렌더링된 원본 HTML
    │     ├─ original.html        # 원본 HTML 백업 (수정 불가 보존)
    │     ├─ structured-data.json # 추출된 JSON-LD
    │     ├─ meta.json            # 클론 메타데이터
    │     │   { "source_url", "cloned_at", "html_hash", "snapshot_id" }
    │     └─ assets/              # 리소스 (선택적)
    │
...
| 정책 | 설명 |
|---|---|
| **원본 백업 보존** | `original.html`은 절대 수정하지 않음 (Before 비교 기준) |
| **작업 사본** | `index.html`이 Optimization Agent의 수정 대상 |
...
```
[Optimization Agent — 클론의 index.html 수정]
    │
...
사용자가 리포트를 검토하고 원본 사이트에 수동 반영한 뒤, 대시보드에서 **[적용 완료 → 재검증]** 을 트리거할 수 있다.

```
[사용자: 원본 사이트에 변경 수동 반영]
    │
    ├─ 대시보드에서 [적용 완료 → 재검증] 클릭
    │
    ├─ Validation Agent: 원본 URL을 LLM에 다시 질의
    │   (LLM 인덱스 갱신 대기 필요 — 최소 24~72시간 권장)
    │
    ├─ 실제 LLM GeoScore 측정
    │   → 초기 기준선(baseline) 대비 실제 개선폭 산출
    │
...
│   │   │   │   ├── defaults/
│   │   │   │   │   ├── orchestrator.ts       # Orchestrator 기본 프롬프트
│   │   │   │   │   ├── analysis.ts           # Analysis Agent 기본 프롬프트
│   │   │   │   │   ├── strategy.ts           # Strategy Agent 기본 프롬프트
│   │   │   │   │   ├── optimization.ts       # Optimization Agent 기본 프롬프트
│   │   │   │   │   ├── validation.ts         # Validation Agent 기본 프롬프트
...
│   │   │   ├── llm/                        # ★ LLM 추상화 레이어 (9-B)
│   │   │   │   ├── provider-config.ts        # LLMProviderConfig, AuthConfig 스키마
...
│   │   │   ├── clone/                     # ★ 클론 관리 (9-C.1)
│   │   │   │   ├── clone-manager.ts          # Clone Manager (원본→로컬 클론 생성)
│   │   │   │   ├── clone-metadata.ts         # 클론 메타데이터 관리
│   │   │   │   └── clone-server.ts           # 로컬 클론 파일 서빙 (구조적 검증용)
│   │   │   ├── report/                    # ★ 결과 리포트 & Archive 생성 (9-C.3)
│   │   │   │   ├── report-generator.ts       # Before-After 비교 리포트 생성
│   │   │   │   ├── archive-builder.ts        # Archive .zip 파일 패키징
│   │   │   │   └── diff-renderer.ts          # Side-by-side diff HTML 렌더링
│   │   │   └── config/
...
│       │   │   ├── clones.ts             # ★ /clones/** 클론 관리 API
│       │   │   ├── reports.ts            # ★ /reports/** 리포트 & Archive 다운로드 API
│       │   │   ├── agents.ts              # /agents/** 에이전트 실행 API
│       │   │   ├── skills.ts              # /skills/** 스킬 관리 API
│       │   │   ├── settings.ts            # ★ /settings/** 프롬프트 CRUD + reset API
│       │   │   └── auth.ts               # ★ /auth/callback OAuth 콜백 + 토큰 관리
│       │   ├── ws/
...
│       │       │   ├── agent-prompt-editor.ts # ★ 에이전트 프롬프트 편집 UI
│       │       │   └── llm-provider-settings.ts # ★ LLM Provider 설정 UI
│       │       └── styles/
│       └── package.json
│
├── workspace/                             # ★ 사용자 작업 공간
...
│   │   ├── orchestrator.json             # 수정 시 생성, 없으면 default fallback
│   │   ├── analysis.json
│   │   ├── strategy.json
│   │   ├── optimization.json
│   │   ├── validation.json
...
│   │   └── {target_id}/                   # Target별 클론 디렉터리
│   │       ├── index.html                 #   작업 사본 (수정 대상)
│   │       ├── original.html              #   원본 백업 (수정 불가)
│   │       ├── structured-data.json       #   JSON-LD 데이터
│   │       ├── meta.json                  #   클론 메타데이터
│   │       └── assets/                    #   리소스 파일
